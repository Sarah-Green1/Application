...
```
Pie chart to investigate if returns are significantly different in different breeds

```{r}
par (mfrow=c(2,2))
par (mar=c(2,2,2,2))
#create a table with relevant data
returned_table <- table(final_clean_data$Breed, final_clean_data$Returned)
#assigning colours to the pie chart segments
breed_colours_2 <- list(
  "Rottweiler" = c("darkred", "lightcoral"),
  "Border Collie" = c("darkgreen", "lightgreen"),
  "Bichon Frise" = c("darkblue", "lightblue")
)
#creating the pie charts, percentages
for (breed in rownames(returned_table)){
  percentages <- round(returned_table[breed,]/sum(returned_table[breed,])*100,1)
pie(returned_table[breed,],
    main = paste(breed, "Return rate"),
    col = breed_colours_2[[breed]],
    labels = c("Not returned", "Returned")
)
    legend(
      x= -2.15, y=1.2,
      legend = paste(c("Not returned", "Returned"), "-", percentages, "%"),
      fill = breed_colours_2[[breed]],
      bty = "n")
}
